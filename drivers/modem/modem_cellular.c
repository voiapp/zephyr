...
int modem_cellular_init(const struct device *dev)
{
	...
	if (modem_cellular_gpio_is_enabled(&config->reset_gpio)) {
		 ...
		 * assertion.  For the normal path, honour hold_reset_on_suspend
		 * to decide whether to assert reset here or release it; the
		 * RESET_PULSE state provides the controlled release otherwise.
		 */
		if (config->autostarts) {
			gpio_pin_configure_dt(&config->reset_gpio, GPIO_OUTPUT_INACTIVE);
		} else {
			gpio_flags_t flags = (config->hold_reset_on_suspend)
						     ? GPIO_OUTPUT_ACTIVE
...
/* startup_time_ms raised to 35000: the EG91-EX (used on doom/wolfenstein) can take up to 35 s
 * after a PWRKEY pulse or hardware reset before CTS is asserted and AT commands are accepted.
 * 15000 ms and 25000 ms were both insufficient for doom's unit, causing every ATE0 attempt to
 * time out because the nRF UART's hw-flow-control blocked TX while CTS was still de-asserted.
 */
#define MODEM_CELLULAR_DEVICE_QUECTEL_EG25_G(inst)                                                 \
	...
	MODEM_CELLULAR_DEFINE_AND_INIT_USER_PIPES(inst,                                            \
						  ...
						  (user_pipe_1, 4))                                \
                                                                                                   \
	MODEM_CELLULAR_DEFINE_INSTANCE(inst, 1500, 500, 35000, 5000, false,                        \
				       NULL,                                                       \
